...
export class LimitOrderComponent implements OnInit {
  ...
  @Input() public orderType = ['Buy','Sell'];
  ...
  ngOnInit() {
    const int = setInterval(() => {
      if (this.mainPair.rate) {
        this.model.Buy.Rate = this.mainPair.rate.toFixed(this.mainPair.basePrecision);
        this.model.Sell.Rate = this.mainPair.rate.toFixed(this.mainPair.basePrecision);
        clearInterval(int);
  ...
  keypad(val) {
    // console.log(this.lastInput.value, val, this.lastInput.value.charAt(0));
    if (val === '.' && (this.lastInput.value === '' || this.lastInput.value === null || this.lastInput.value.indexOf('.') > -1)) {
      return;
    }
    // console.log(this.cursorPosition, this.lastInput.selectionStart,val);
    if (this.lastInput === undefined || (this.lastInput.value.charAt(0) === '.' && val === '.')) {
      return;
    }
    // console.log(this.cursorPosition,this.lastInput.selectionStart);
    const a = this.lastInput.value;
    const b = val;
    const position = this.lastInput.selectionStart;
    const output = [a.slice(0, position), b, a.slice(position)].join('');
    // console.log(position);
    this.lastInput.value = output; // this.lastInput.value + val;
  ...
  inputChange(ordrType: string, buy: boolean, type, e) {
    ...
    // this.model[ordrType].Rate = Number(this.model[ordrType].Rate).toFixedFloor(this.precisionBC);
      ...
      if (type === 'r') {
        if (t[1]) {
          ...
          if (t[1].length >= (this.precisionBC)) {
            ...
            // console.log(Number(this.decimallength),Number(this.decimallength).toFixed(this.precisionBC));
    ...
    if (ordrType === 'Buy') {
      if ((this.model[ordrType].Quantity !== 0 && this.model[ordrType].Quantity !== null) &&
        ...
        this.buyqt = ((this.model[ordrType].Quantity * this.model[ordrType].Rate).toFixedFloor(this.precisionBC));
      } else {
        this.buyqt = null;
      }
    } else {
      if ((this.model[ordrType].Quantity !== 0 && this.model[ordrType].Quantity !== null) &&
        ...
        this.sellqt = ((this.model[ordrType].Quantity * this.model[ordrType].Rate).toFixedFloor(this.precisionBC));
      } else {
        this.sellqt = null;
      }
    }
    ...
    if (ordrType === 'Buy') {
      if (isNaN(this.model[ordrType].Quantity)) {
        this.model[ordrType].Quantity = '';
      }
      if (isNaN(this.buyqt)) {
        this.buyqt = '';
      }
      // if (this.isNumber(this.model[ordrType]) || this.isNumber(this.model[ordrType].Quantity)) {
      //   this.buyqt = '';
      // }
    }
  ...
  amountChange(type, ordrType, changetype) {
    ...
    if (ordrType === 'Buy') {
      this.buyqt = Number((Number(this.model[ordrType].Quantity) *
        Number(this.model[ordrType].Rate)).toFixedFloor(this.precisionBC)).toFixedFloor(this.precisionBC);
    } else {
      this.sellqt = Number((Number(this.model[ordrType].Quantity) *
        Number(this.model[ordrType].Rate)).toFixedFloor(this.precisionBC)).toFixedFloor(this.precisionBC);
    }
  ...
  increment(type: any, ordrType: any) {
    if (type === 'r') {
  ...
  //   this.decimallength = this.lastInput.value;
  //   const evt = document.createEvent('HTMLEvents');
  //   evt.initEvent('input', false, true);
  //   this.lastInput.dispatchEvent(evt);
  // }
  ...
  keypadRemove() {
    ...
    // console.log(this.lastInput.value, this.lastInput.selectionStart);
    // console.log(this.lastInput.value.match('([A-Z,a-z])\w+'));
    ...
    // console.log(
    //   this.lastInput.value,
    //   Number(this.lastInput.value),
    //   this.lastInput.selectionStart
    // );
    ...
    // var a = this.lastInput.value
    // var b = val;
    // var position = this.lastInput.selectionStart;
    // var output = [a.slice(0, position), b, a.slice(position)].join('');
    // console.log(Number(lastValue));
    // this.lastInput.value = Number(lastValue);
    // console.log(this.lastInput.value,'final');
    const evt = document.createEvent('HTMLEvents');
    evt.initEvent('input', false, true);
    this.lastInput.dispatchEvent(evt);
    // console.log(this.lastInput.selectionStart,'before',this.cursorPosition);
    this.lastInput.selectionStart = this.lastInput.selectionStart - 1;
    this.cursorPosition = this.cursorPosition - 1;
    // console.log(evt, "after");
    // this.lastInput.value = parseFloat(lastValue);
    this.lastInput.value = lastValue;
    // evt.target.value = Number(lastValue);
    this.onFocus(evt);
  }
  upAndDownKey(e, type, ordrType) {
    // return;
    if (type === 'r') {
      e.target.value = Number(this.model[ordrType].Rate).toFixed(this.precisionBC);
      // this.model[ordrType].Rate = Number(this.model[ordrType].Rate).toFixed(this.precisionBC);
    }
  }
  // validateKeyDown(e: any) {
  // }
  ...
  setAll(pair: Pair) {
    this.model.Buy.Rate = pair.rate.toFixedFloor(pair.basePrecision);
    this.model.Sell.Rate = pair.rate.toFixedFloor(pair.basePrecision);
  ...
  applyPercent(input: number, buy: boolean, ordertype) {
    if (!this.isLogin || ordertype === undefined) {
      return;
    }
